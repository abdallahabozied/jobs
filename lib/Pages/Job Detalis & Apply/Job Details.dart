              ...
              Container(
                ...
                decoration: BoxDecoration(
                    image: DecorationImage(
                        image: NetworkImage("${list3[prov.postid]["thumbnailUrl"]}"),
                        fit: BoxFit.fill),
                    borderRadius: BorderRadius.circular(20)),
                //  child:,
              ),
              Title(color: Colors.black, child: Text("${list3[prov.postid]["title"]}",style: const TextStyle(fontSize: 25,fontWeight: FontWeight.w400),)),
              Text("${list3[prov.postid]["id"]}",style: const TextStyle(color: Colors.black54,fontSize: 20),),
              SizedBox(
                width: 220,
                child: Row(
                    ...
                    child: TabBarView(controller: _tabController, children: [
                      SingleChildScrollView(
                          ...
                          child: RichText(
                            ...
                            TextSpan(
                                text:("${list3[prov.postid]["subtitle"]}"),
                                style: const TextStyle(color: Colors.black)),
                            TextSpan(
                                text:
                                ("${list3[prov.postid]["subtitle"]}"),
                                style: const TextStyle(color: Colors.black)),
                            const TextSpan(
                                text: "Skill Required \n",
                                style:
                            ...
                            TextSpan(
                                text:
                                     ("${list3[prov.postid]["id"]} \n"),
                                style: const TextStyle(color: Colors.black)),
                            TextSpan(
                                text:
                                ("${list3[prov.postid]["id"]+1} \n"),
                                style: const TextStyle(color: Colors.black)),
                            TextSpan(
                                text:
                                ("${list3[prov.postid]["id"]+2} \n"),
                                style: const TextStyle(color: Colors.black)),
                            TextSpan(
                                text:
                                ("${list3[prov.postid]["id"]+3} \n"),
                                style: const TextStyle(color: Colors.black)),
                          ]))),
                      SingleChildScrollView(
                          physics: const BouncingScrollPhysics(),
                      ...
                      SizedBox(
                        ...
                        child: ListView.builder(itemCount: 30,itemBuilder: (context, index) {
                          return  Column(
                            children: [
                              ListTile(
                                leading: CircleAvatar(radius: 22,backgroundImage:NetworkImage("${list3[prov.postid]["thumbnailUrl"]}")),
                                title: Text("${list3[prov.postid]["id"]}",style:const TextStyle(fontWeight: FontWeight.w500,color: Colors.black)),
                                subtitle: Text("${list3[prov.postid]["thumbnailUrl"]}",style: TextStyle(fontWeight: FontWeight.w400,color: Colors.grey[600])),
                                trailing:Column(
                                  mainAxisAlignment: MainAxisAlignment.center,
                                  crossAxisAlignment: CrossAxisAlignment.end,
                                  children: [
                                    const Text("Work During",style: TextStyle(color: Colors.grey)),
                                    Text("${list3[prov.postid]["id"]+1}",style: TextStyle(color: Colors.blue[700]))
                                  ],
                                ),
